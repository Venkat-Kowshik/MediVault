...
function print(){
	...
	.then(()=>{
		...
		wbc.textContent='Platelets : '+searchData('platelet',answer)
...
function searchData(value, text) {
  ...
  function getInsightsHelper(){
	let mode=modeSelector.value
	let unit=''
	console.log(reparray)
	if(mode=='rbc')
		unit='RBC Count (10^6/cu.mm) values: '
	else if(mode=='hmo')
		unit='Hemoglobin (g/dL) values: '
	else
		unit='Platelet count (10^3/µL) values: '
	let query=unit;
	reparray.forEach(i=>{
		query+=i[mode]+','
	})
  ...
  async function getInsights(text) {
	const encodedText = encodeURIComponent(text);
	try {
	  const response =  await fetch(`http://127.0.0.1:8000/getinsights?text=${encodedText}`);
	  body = await response.json()
	  if(response.ok){
		// console.log(response)
		// console.log(response.reply)
		console.log(body)
		aiContainer.style.cssText='display: block; margin-bottom: 10px; text-align: center;'
		aiText.textContent=body.reply
	  }
	  else
		console.log(response)
	}
